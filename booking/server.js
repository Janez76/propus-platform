...
const bookingSessionStore = bookingSessionPool
  ...
  : null;
app.use(session({
  name: "propus_booking.sid",
  secret: bookingSessionSecret,
  proxy: true,
  resave: false,
...
async function requireCustomer(req, res, next) {
  try {
    ...
    if (!token) {
      token = String(req.query.token || req.query.magic || "").trim();
    }
...
function appendCustomerAuthQueryParam(baseUrl, key, value) {
  try {
    const u = new URL(String(baseUrl || "").trim() || "https://booking.propus.ch/");
    u.searchParams.set(key, String(value));
    return u.toString();
  } catch {
    const b = String(baseUrl || "");
    const sep = b.includes("?") ? "&" : "?";
    return `${b}${sep}${encodeURIComponent(key)}=${encodeURIComponent(String(value))}`;
  }
}
...
function safeCustomerMagicReturnPath(raw) {
  ...
  if (s.startsWith("/")) {
    ...
    return s.length > 2048 ? fallback : s;
  }
  try {
...
function absoluteCustomerRedirectTarget(pathOrUrl) {
  const feBase = resolveCustomerFrontendRedirect("/").replace(/\/?$/, "/");
  const p = String(pathOrUrl || "/").trim() || "/";
  if (p.startsWith("http://") || p.startsWith("https://")) return p;
  try {
    return new URL(p.startsWith("/") ? p : `/${p}`, feBase).toString();
  } catch {
    return feBase;
  }
}
...
app.get("/auth/customer/magic", async (req, res) => {
  try {
    if (!process.env.DATABASE_URL) {
      return res.redirect(
        302,
        appendCustomerAuthQueryParam(resolveCustomerFrontendRedirect("/"), "auth_error", "db_unavailable"),
      );
    }
    const raw = String(req.query.magic || req.query.impersonate || req.query.token || "").trim();
    if (!raw) {
      return res.redirect(
        302,
        appendCustomerAuthQueryParam(resolveCustomerFrontendRedirect("/"), "auth_error", "missing_token"),
      );
    }
    const tokenHash = customerAuth.hashSha256Hex(raw);
    const customer = await db.getCustomerBySessionTokenHash(tokenHash);
    if (!customer || customer.blocked) {
      return res.redirect(
        302,
        appendCustomerAuthQueryParam(resolveCustomerFrontendRedirect("/"), "auth_error", "invalid_session"),
      );
    }
    const days = Math.max(1, Number(CUSTOMER_SESSION_DAYS) || 30);
    const maxAgeMs = days * 24 * 60 * 60 * 1000;
    res.cookie("customer_session", raw, customerSessionCookieOptions(maxAgeMs));
    const targetPath = safeCustomerMagicReturnPath(req.query.returnTo);
    res.redirect(302, absoluteCustomerRedirectTarget(targetPath));
  } catch (err) {
    console.error("[auth/customer/magic]", err?.message || err);
    return res.redirect(
      302,
...
// Legacy-Kompatibilitaet: alte SSO-/Auth-URLs (/auth/customer/*) ins Frontend umleiten
...
app.post("/api/admin/orders/:orderNo/storage/nextcloud-share", requireAdmin, async (req, res) => {
  try {
    if (!process.env.DATABASE_URL) return res.status(503).json({ error: "DB nicht verfuegbar" });
    const orderNo = Number(req.params.orderNo);
    const folderType = String(req.body?.folderType || "customer_folder");
    if (folderType !== "customer_folder") {
      return res.status(400).json({ error: "Nextcloud-Share nur fuer customer_folder unterstuetzt" });
    }
    const order = await db.getOrderByNo(orderNo);
    if (!order) return res.status(404).json({ error: "Order not found" });
    const {
    ...
    if (!isNextcloudConfigured()) {
      return res.status(503).json({
        error: `${getNextcloudConfigError()}. Nextcloud läuft auf dem UGREEN NAS (192.168.1.5); bei lokalem Start bitte NEXTCLOUD_URL, NEXTCLOUD_USER und NEXTCLOUD_PASS in .env, .env.local oder .env.vps setzen.`,
      });
    }
...
// Portal-Firmen feature removed: customers no longer have a company scope.
// Keep helper signature for customer-contacts-routes / exxas-reconcile-routes;
// always allow since there is no company filter anymore.
async function ensureCustomerInRequestCompany(_req, _customerId) {
  return true;
}
...
const ADMIN_PROFILE_LANGUAGES = new Set(["de", "en", "fr", "it"]);

function isEmailLike(value) {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(value || "").trim());
}

function normalizeAdminProfileLanguage(value) {
  const lang = String(value || "").trim().toLowerCase();
  return ADMIN_PROFILE_LANGUAGES.has(lang) ? lang : "de";
}

async function resolveCurrentAdminUserRecord(req) {
  const directId = Number(req.user?.userKey || req.user?.id);
  if (Number.isFinite(directId) && directId > 0 && db.getAdminUserById) {
    const byId = await db.getAdminUserById(directId);
    if (byId) return byId;
  }
  ...
  for (const candidate of candidates) {
    const record = await db.getAdminUserByUsername(String(candidate));
...
app.put("/api/admin/me", requireAdmin, async (req, res) => {
  try {
    ...
    const language = normalizeAdminProfileLanguage(req.body?.language || currentProfile.language);
    ...
    if (token) {
      const tokenHash = customerAuth.hashSha256Hex(token);
      const nextSessionUserKey = isEmailLike(req.user?.userKey) ? email : (req.user?.userKey || null);
      await db.query(
        "UPDATE admin_sessions SET user_key = $1, user_name = $2 WHERE token_hash = $3",
        [nextSessionUserKey, name, tokenHash]
      ).catch(() => null);
    }

    req.user = {
      ...(req.user || {}),
      userKey: isEmailLike(req.user?.userKey) ? email : (req.user?.userKey || email),
      email,
      name,
    };
...
// (?lat=&lng=) übergeben — Letzteres wird vom Auftrags-Popover genutzt,
// damit jede Buchung das Wetter ihrer Adresse sieht.
const weatherProvider = require("./weather-openmeteo");
...
function _wxResolveCoords(req) {
  const lat = Number(req.query.lat);
  const lng = Number(req.query.lng ?? req.query.lon);
  if (Number.isFinite(lat) && Number.isFinite(lng)) {
    return { lat, lng, region: { key: "custom", name: "", lat, lng } };
  }
  const key = String(req.query.region || "zurich").trim().toLowerCase();
  const regionKey = WEATHER_REGIONS[key] ? key : "zurich";
  const region = WEATHER_REGIONS[regionKey];
  return { lat: region.lat, lng: region.lng, region: { key: regionKey, ...region } };
}

app.get("/api/admin/weather/forecast", requirePhotographerOrAdmin, async (req, res) => {
  try {
    const fromRaw = String(req.query.from || "").trim();
    const fromIso = /^\d{4}-\d{2}-\d{2}$/.test(fromRaw) ? fromRaw : null;
    const days = Math.max(1, Math.min(16, Number(req.query.days || 7) | 0 || 7));
    const { lat, lng, region } = _wxResolveCoords(req);
    const out = await weatherProvider.fetchDaily(lat, lng, fromIso, days);
    res.set("Cache-Control", "public, max-age=900");
    res.json({ ok: true, region, days: out });
  } catch (err) {
    console.error("[weather/forecast] error:", err?.message || err);
    res.status(502).json({ error: err.message || "Wetterdaten konnten nicht geladen werden" });
  }
});

app.get("/api/admin/weather/hourly", requirePhotographerOrAdmin, async (req, res) => {
  try {
    const dateIso = String(req.query.date || "").trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(dateIso)) {
      return res.status(400).json({ error: "Parameter `date` fehlt oder ist nicht im Format YYYY-MM-DD." });
    }
    const { lat, lng, region } = _wxResolveCoords(req);
    const hours = await weatherProvider.fetchHourly(lat, lng, dateIso);
    res.set("Cache-Control", "public, max-age=900");
    res.json({ ok: true, region, date: dateIso, hours });
  } catch (err) {
    console.error("[weather/hourly] error:", err?.message || err);
    res.status(502).json({ error: err.message || "Wetterdaten konnten nicht geladen werden" });
  }
});
...
// Company Workspace (Portal-Firmen) — ENTFERNT (Logto-Abbau, April 2026).
// Alle /api/company/*, /api/admin/companies/*, /api/admin/users/companies/* sowie
// die zugehörigen Helper und Logto-Sync wurden entfernt.

// ─── Interne Benutzer (lokal) ────────────────────────────────────────────────
...
// Aktivit?tslog eines Mitarbeiters
...
// Abwesenheits-Kalender-Eintr?ge abrufen
...
app.get("/api/admin/backup-config", requireAdmin, (req, res) => {
  ...
  try {
    if (fs.existsSync(nasLogPath)) {
      const log = fs.readFileSync(nasLogPath, "utf8").trim();
      const lines = log.split("\n").filter(Boolean);
      const last = lines[lines.length - 1] || "";
      lastSync = last;
      lastSyncStatus = last.includes("abgeschlossen") ? "ok" : last.includes("FEHLER") ? "error" : "unknown";
    }
  } catch (_) {}
  ...
  try {
    ...
    async function dirContents(dirPath) {
      const known = {
        ...
        "state.tar.gz": "State-Volume",
        "logs.tar.gz": "Log-Volume",
        "upload_staging.tar.gz": "Upload-Staging",
        "booking_upload_customer.tar.gz": "Kunden-Uploads",
        "booking_upload_raw.tar.gz": "Raw-Uploads",
...
app.post("/api/admin/backups/create", requireAdmin, async (req, res) => {
  try {
    ...
    const scriptPath = process.env.BACKUP_SCRIPT_PATH || "/app/scripts/backup-vps.sh";
    if (!fs.existsSync(scriptPath)) {
      return res.status(500).json({
        error: `Backup-Script nicht gefunden: ${scriptPath}`,
        hint: "Setze BACKUP_SCRIPT_PATH korrekt oder deploye das Backend neu.",
      });
    }
    ...
    execFile("/bin/sh", [scriptPath], { timeout: 300000, env }, (err, stdout, stderr) => {
      if (err) return res.status(500).json({ error: err.message, stderr, stdout, scriptPath });
...
app.post("/api/admin/backups/:name/restore", requireAdmin, async (req, res) => {
  try {
    ...
    const pgDb = process.env.POSTGRES_DB || "propus";
    const pgUser = process.env.POSTGRES_USER || "propus";
    const pgPass = process.env.POSTGRES_PASSWORD || "";

    function restoreSQL(sqlPath, host, port, user, db, pass) {
      return new Promise((resolve, reject) => {
        const env = { ...process.env, PGPASSWORD: pass };
        const psql = spawn("psql", ["-h", host, "-p", port, "-U", user, "-d", db], { env });
        let stdout = "";
        let stderr = "";
        ...
        psql.on("error", (err) => reject({ message: err.message, stderr }));
        psql.on("close", (code) => {
          if (code === 0) resolve(stdout);
          else reject({ message: `psql exited with code ${code}`, stderr });
        });

        const stream = fs.createReadStream(sqlPath, { encoding: "utf8" });
        const rl = readline.createInterface({ input: stream, crlfDelay: Infinity });
        rl.on("line", (line) => {
          if (line.trim() === "SET transaction_timeout = 0;") return;
        ...
        rl.on("error", (err) => {
          try { psql.kill(); } catch (_) {}
          reject({ message: err.message, stderr });
        });
      });
    }

    function restoreArchive(archivePath, targetPath) {
      return new Promise((resolve, reject) => {
        ...
        execFile(
          "tar",
          ["-xzf", archivePath, "-C", parentDir],
          { timeout: 600000 },
          (err, stdout, stderr) => {
            if (err) reject({ message: err.message, stderr });
            else resolve(stdout);
          }
        );
      });
    ...
    if (name.endsWith(".sql")) {
      try {
        const out = await restoreSQL(fullPath, pgHost, pgPort, pgUser, pgDb, pgPass);
        return res.json({ ok: true, message: "SQL-Dump wiederhergestellt", output: out });
      } catch (e) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      const volumeMappings = [
        ...
        { file: "upload_staging.tar.gz", target: process.env.BOOKING_UPLOAD_STAGING_ROOT || "/upload_staging" },
        { file: "booking_upload_customer.tar.gz", target: process.env.BOOKING_UPLOAD_CUSTOMER_ROOT || "/booking_upload_customer" },
        { file: "booking_upload_raw.tar.gz", target: process.env.BOOKING_UPLOAD_RAW_ROOT || "/booking_upload_raw" },
      ];

      if (!fs.existsSync(dbSql)) {
        return res.status(400).json({ error: "db.sql nicht im Backup-Ordner gefunden" });
      }

      const results = [];
      try {
        const out = await restoreSQL(dbSql, pgHost, pgPort, pgUser, pgDb, pgPass);
      ...
      if (req.body?.restoreVolumes !== false) {
        for (const mapping of volumeMappings) {
          const archivePath = path.join(fullPath, mapping.file);
          if (!fs.existsSync(archivePath)) continue;
          try {
            const out = await restoreArchive(archivePath, mapping.target);
            results.push({ volume: mapping.target, file: mapping.file, ok: true, output: out });
          } catch (e) {
            results.push({ volume: mapping.target, file: mapping.file, ok: false, error: e.message, stderr: e.stderr });
          }
        }
      }
      ...
      return res.json({
        ...
        warning: req.body?.restoreVolumes !== false
          ? "Volume-Restore wurde im laufenden Betrieb angewendet. Fuer einen strikt konsistenten Voll-Restore ist restore-vps.sh auf dem VPS vorzuziehen."
          : undefined,
        results
      });
    }

    res.status(400).json({ error: "Backup-Typ nicht unterstuetzt" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
