          ...
          {suggestions.map((item, index) => (
            ...
            >
              <div className="flex items-start gap-2">
                <User className="mt-[2px] h-3.5 w-3.5 text-[#C5A059]" />
                <div className="min-w-0">
                  <p className="truncate font-semibold text-slate-900 dark:text-zinc-100">
                    {String(item.company || "").trim() || item.name || "-"}
                  </p>
                  {item.company && item.name ? (
                    <p className="truncate text-xs text-slate-500 dark:text-zinc-400">{item.name}</p>
                  ) : null}
                  {item.id != null ? (
                    <p className="text-[11px] font-medium tabular-nums text-slate-400 dark:text-zinc-500">
