...
const STATUS_LABELS_DE = {
  ...
  [ORDER_STATUS.PAUSED]:            "Wartet auf Kunde",
  [ORDER_STATUS.COMPLETED]:         "Material in Bearbeitung",
 ...
 *
 * Admin-Workflow-Prinzip:
 * Im Kanban kann jeder Auftrag von jeder Bucket-Spalte in jede andere
 * gezogen werden — der Admin entscheidet. Backend prueft weiterhin die
 * harten Invarianten:
 *  - INV-SM-04 / INV-SM-05: confirmed + provisional erfordern Fotograf
 *    UND Termin (Pruefung in state-machine.getTransitionError).
 *  - provisional -> pending nur via expiry_job (Provisorium laeuft ab).
 * Sonst ist die Matrix bewusst liberal, damit die UI keine Drops
 * vortaeuscht die das Backend dann ablehnt.
 */
const ALLOWED_TRANSITIONS = {
  ...
  // provisional -> pending nur via expiry_job (siehe isTransitionAllowed)
  [ORDER_STATUS.PROVISIONAL]:       [ORDER_STATUS.CONFIRMED, ORDER_STATUS.DISPOSITION_OFFEN, ORDER_STATUS.PAUSED, ORDER_STATUS.COMPLETED, ORDER_STATUS.DONE, ORDER_STATUS.CANCELLED, ORDER_STATUS.ARCHIVED],
  [ORDER_STATUS.DISPOSITION_OFFEN]: [ORDER_STATUS.PENDING, ORDER_STATUS.PROVISIONAL, ORDER_STATUS.CONFIRMED, ORDER_STATUS.PAUSED, ORDER_STATUS.COMPLETED, ORDER_STATUS.DONE, ORDER_STATUS.CANCELLED, ORDER_STATUS.ARCHIVED],
  [ORDER_STATUS.PAUSED]:            [ORDER_STATUS.PENDING, ORDER_STATUS.PROVISIONAL, ORDER_STATUS.DISPOSITION_OFFEN, ORDER_STATUS.CONFIRMED, ORDER_STATUS.COMPLETED, ORDER_STATUS.DONE, ORDER_STATUS.CANCELLED, ORDER_STATUS.ARCHIVED],
  [ORDER_STATUS.CONFIRMED]:         [ORDER_STATUS.PENDING, ORDER_STATUS.PROVISIONAL, ORDER_STATUS.DISPOSITION_OFFEN, ORDER_STATUS.COMPLETED, ORDER_STATUS.DONE, ORDER_STATUS.PAUSED, ORDER_STATUS.CANCELLED, ORDER_STATUS.ARCHIVED],
  [ORDER_STATUS.COMPLETED]:         [ORDER_STATUS.PENDING, ORDER_STATUS.CONFIRMED, ORDER_STATUS.DONE, ORDER_STATUS.PAUSED, ORDER_STATUS.CANCELLED, ORDER_STATUS.ARCHIVED],
  [ORDER_STATUS.DONE]:              [ORDER_STATUS.PENDING, ORDER_STATUS.CONFIRMED, ORDER_STATUS.COMPLETED, ORDER_STATUS.PAUSED, ORDER_STATUS.CANCELLED, ORDER_STATUS.ARCHIVED],
  [ORDER_STATUS.CANCELLED]:         [ORDER_STATUS.PENDING, ORDER_STATUS.CONFIRMED, ORDER_STATUS.COMPLETED, ORDER_STATUS.DONE, ORDER_STATUS.PAUSED, ORDER_STATUS.ARCHIVED],
  [ORDER_STATUS.ARCHIVED]:          [ORDER_STATUS.PENDING, ORDER_STATUS.CONFIRMED, ORDER_STATUS.COMPLETED, ORDER_STATUS.DONE, ORDER_STATUS.PAUSED, ORDER_STATUS.CANCELLED],
};
