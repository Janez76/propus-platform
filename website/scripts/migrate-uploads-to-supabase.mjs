#!/usr/bin/env node
/**
 * Alle CMS-Referenzen `/uploads/cms/<datei>` → öffentliche Supabase-Storage-URL.
 * - CMS-Payload nur aus Supabase (cms_state); kein data/cms.json mehr.
 * - Liest jede so referenzierte Datei von der Platte und lädt sie hoch (falls vorhanden).
 * - Liegt die Datei nur schon im Bucket, wird nur die URL im Payload ersetzt.
 * - Läuft auch, wenn der Ordner leer ist, aber noch Pfade im CMS stehen (Bucket-Abgleich).
 *
 * Voraussetzung: .env mit SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY, Bucket „cms“.
 * Aufruf: npm run migrate:uploads
 */
...
/** Alle Dateinamen, die im Payload als exakter String `/uploads/cms/name` vorkommen. */
...
async function main() {
	...
	if (!url || !key) {
		console.error('Fehlt SUPABASE_URL oder SUPABASE_SERVICE_ROLE_KEY (.env + npm run migrate:uploads).');
		process.exit(1);
	}
	...
	const { data: row, error: readErr } = await supabase.from('cms_state').select('payload').eq('id', 1).maybeSingle();
	if (readErr) {
		console.error('Supabase cms_state lesen:', readErr.message);
		process.exit(1);
	}
	if (!isValidCmsPayload(row?.payload)) {
		console.error(
			'Kein gültiges CMS in Supabase (Zeile cms_state id=1, version/media). Zuerst Migration SQL ausführen und ggf. Admin/Seed nutzen.',
		);
		process.exit(1);
	}
	const payload = structuredClone(row.payload);
	console.log('CMS geladen von: Supabase');
	...
	console.log(
		`Referenzen im CMS: ${refNames.size}, Dateien lokal: ${diskNames.size}, zusammen eindeutig: ${toProcess.size}`,
	);

	let inBucket;
	try {
		inBucket = await listBucketFilenames(supabase);
		console.log(`Dateien bereits im Storage-Bucket: ${inBucket.size}`);
	} catch (e) {
	...
	for (const name of toProcess) {
		...
		if (fromDisk) {
			...
			console.log(`Hochgeladen (lokal): ${oldPath}`);
			continue;
		}
		...
		if (inBucket.has(name)) {
			const { data: pub } = supabase.storage.from(BUCKET).getPublicUrl(name);
			urlMap[oldPath] = pub.publicUrl;
			console.log(`Nur URL ersetzen (schon im Bucket): ${name}`);
		} else if (refNames.has(name)) {
			console.warn(`WARNUNG: CMS verweist auf ${oldPath}, Datei fehlt lokal und im Bucket.`);
		}
	}
	...
	console.log(`CMS-Pfade ${PREFIX}: vorher ${beforeRefs}, nachher ${afterRefs}`);

	const payloadChanged = JSON.stringify(updated) !== JSON.stringify(payload);

	if (!payloadChanged && uploadedFromDisk.length === 0) {
		console.log('Nichts zu tun (keine /uploads/cms/-Referenzen, keine lokalen Uploads).');
		process.exit(0);
	}
	...
	if (payloadChanged) {
		...
		if (writeErr) {
			console.error('Supabase cms_state speichern fehlgeschlagen:', writeErr.message);
			process.exit(1);
		}
		console.log('CMS-Payload in Supabase gespeichert.');
	}
	...
	if (uploadedFromDisk.length) {
		console.log(`${uploadedFromDisk.length} lokale Datei(en) nach Upload entfernt.`);
	}

	if (failed.length) {
		console.warn(`${failed.length} Upload(s) fehlgeschlagen.`);
		process.exit(1);
	}
}
