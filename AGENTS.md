...
- **Propus-Assistant (Admin-Chat):** Keine eingebundene Live-Wetter-, Routing- oder Geocode-API — Verhalten in **`app/src/lib/assistant/system-prompt.ts`** und bei Bedarf **`app/src/lib/assistant/few-shot-examples.ts`**; Schweiz-Wetter: Nutzer auf **MeteoSchweiz** verweisen; Fahrzeiten/Distanzen nur als grobe Schätzung plus Hinweis auf Karten-Apps (Regression: Eval-Fälle **`weather-honest`** und **`routing-honest`** in **`app/scripts/eval-assistant.ts`**).
