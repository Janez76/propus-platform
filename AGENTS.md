...
- **PowerShell & UNC/SMB:** `&&` für Verkettung unzuverlässig — stattdessen `;` oder `if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }`; **mehrzeilige Git-Commits:** Here-String (`$msg = @'...'@; git commit -m $msg`). Längere **`npm`/`vitest`/`gradle`**: lieber von gemappten Laufwerken (z. B. `Y:`) als von UNC; auf Netzlaufwerken können Jobs hängen. **NAS-Ordner-Duplikate** (NTFS/Unicode): `scripts/diagnose-nas-*`.
...
- **Propus-Assistant (Admin-Chat):** Wetter über **`get_weather_forecast`** / **`get_weather_for_order`** (Open-Meteo · MeteoSwiss ICON-CH; bis **7** Tage mit Parameter **`days`**). **MeteoSchweiz** als Behörde bietet **keine** einfache öffentliche Forecast-REST zum Einbinden; amtliche Warnungen: **https://www.meteoschweiz.admin.ch**. Routing primär **`get_route`** / **`get_distance_matrix`** (Regel **7b**); nur bei Tool-Fehler/Nichtkonfiguration grobe Schätzung + Karten-App („ROUTING UND ENTFERNUNGEN“). Prompt/Few-Shots: **`app/src/lib/assistant/system-prompt.ts`**, **`few-shot-examples.ts`**; Regression: **`weather-honest`**, **`routing-honest`** in **`app/scripts/eval-assistant.ts`**. Trainings-/Eval-Workflow: **`app/scripts/README-training.md`**; npm **`eval:assistant`**, **`tune:assistant`**, **`replay:conversations`**, **`seed:memories`**; z. B. **`ASSISTANT_REPLAY_USER_ID`** (Replay-Harvest), **`ASSISTANT_SEED_USER_ID`** (YAML-Seeds mit Platzhalter-`userId`). **DB-Tools** (z. B. `search_customers`, `search_invoices`): **`core.customers`** nutzt **`notes`** / **`street`** (nicht `notiz` / Spalte `address`). View **`tour_manager.invoices_central_v`**: u. a. **`invoice_source`**, **`amount_chf`**, **`tour_customer_name`**, **`tour_object_label`**.
- **Galerie-/Listing-Modul:** Backend **`tours/lib/gallery.js`** (Erstellen, NAS-Import, Bildverwaltung, Deduplizierung, Thumbnails); Admin-API **`tours/routes/gallery-admin-api.js`** (CRUD + Thumb-Serving via `/api/tours/admin/galleries/:id/images/:imgId/thumb`); Public-API **`tours/routes/gallery-public-api.js`** (Download, ZIP mit Galerie-Titel als Dateiname); Frontend **`app/src/pages-legacy/admin/listing/ListingEditorPage.tsx`**. Beim NAS-Import werden nur Bilder/Feedback neu geladen, Formulardaten (Titel, Adresse, E-Mail) bleiben erhalten. **Quick-Hot-Patch** Express-only: `scp` nach VPS `/tmp/` dann `docker cp` in Container und `docker restart` (kein Next.js-Rebuild noetig).
- **Newsletter (Website):** Seit Mai 2026 ueber **MailerLite** (vorher Mailchimp). Subscribe-Endpoint: **`website/src/pages/api/newsletter/subscribe.ts`**; Popup: **`website/src/components/NewsletterPopup.astro`** (einmalig per `localStorage` Key `propus_nl_popup_v1`). Env: **`MAILERLITE_API_KEY`**, **`MAILERLITE_NEWSLETTER_GROUP_ID`** (in `docker-compose.vps.yml` + `.env.vps`). Migrationsskripte unter `scripts/`: `mailchimp-audit.ts`, `mc-to-ml-migrate.ts`, `mc-to-ml-suggest-mapping.ts`; shared Lib `scripts/lib/mailerlite-fetch.ts`.
