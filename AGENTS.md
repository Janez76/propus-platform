...
- Buchungsflow: bei Bedarf **ohne festes Datum und Uhrzeit** buchen können (kein Kalendereintrag bzw. als Tages-/Follow-up-Aufgabe); Termin **absagbar** und mit dem Kunden **abzusprechen** — wiederkehrendes Produktziel, nicht nur Einzelfall.
...
**BKBN-Kalender (fremde Postfächer, `calendarView`):** Permission **Calendars.Read** (application), Admin-Consent, optional Exchange Application Access Policy; VPS `BKBN_CALENDAR_MAILBOXES`; Verifikation — **`docs/OPS_M365_GRAPH_BKBN.md`**.
...
- **Platform-Docker:** Next.js ~**3001**, Express ~**3100** (`PLATFORM_INTERNAL_URL`); Health-Checks auf **`/api/core/health`**. Nach Next-Deploy kann **`ChunkLoadError`** bei **`React.lazy()`** auftreten → **`ChunkErrorBoundary`** um **`Suspense`/`Routes`** in **`ClientShell`**.
