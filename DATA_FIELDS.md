...
*Zuletzt aktualisiert: Mai 2026 (PR #443: Self-Serve Magic-Link Login fuer Kunden — neue Tabelle `core.customer_login_tokens` (Migration 064), Endpoints `POST/GET /api/customer/magic-link/{request,callback}`, Toggle in LoginPage (Magic-Link primaer auf portal.*, Passwort weiterhin verfuegbar); Doku in `docs/FLOWS_AUTH.md` §5b, `docs/SCHEMA_FULL.md`, `docs/EMAIL_TEMPLATES.md`. Roadmap fuer spaetere `tour_manager.portal_*` -> `core.company_members` Konsolidierung in `docs/PLAN_TOUR_MANAGER_CONSOLIDATION.md`). April 2026: `DEPLOY-FLOW.md` Deploy-Trigger + CI-Matrix; `FLOWS_BOOKING.md` §19 booking-ci. PR #96: Lücke #6 behoben — FLOWS_EXXAS.md §3 Sync-Häufigkeit + §7 Order-Sync-Status dokumentiert. PR #95: booking.companies/company_members/company_invitations per Migration 082 nach core.* gemergt und gedroppt; FKs booking.orders.created_by_member_id + booking.access_subjects.company_member_id zeigen jetzt auf core.company_members). PR #94: admin_users Legacy-Tabellen durch Views mit INSTEAD-OF-Triggern über `core.admin_users` ersetzt; FK `booking.access_subjects.admin_user_id` zeigt jetzt auf `core.admin_users(id)`). PR #93: `booking/admin-panel/` vollständig entfernt — Code, Docker-Services, CI-Build-Step, Warn-Workflow gelöscht; Docs bereinigt. PR #92: API-Key-Auth bindet req.user.id auf numerische admin_users.id, created_by-Lookup per ID. PR #91: API-Token-Generator — core.api_keys-Tabelle, CRUD-Endpunkte, RBAC-Permission `api_keys.manage`, Settings-UI. PR #90: booking/Dockerfile Build-Context auf Repo-Root, core/ wird mitkopiert wegen cross-module Dependency. PR #89: Rate-Limiting auf Auth/Booking-Endpunkte, helmet Security-Header, OpenAPI-CI-Lint, SCHEMA_FULL.md um ~20 fehlende Tabellen ergänzt. PR #88: Node-Pinning auf 20.18.1, GOOGLE_REVIEWS_PLACE_ID externalisiert, core/lib/customer-lookup zentralisiert.*
...
| Datei | Inhalt |
|---|---|
| [docs/FLOWS_BOOKING.md](docs/FLOWS_BOOKING.md) | Buchungs-Flows: Haupt-Buchung, Provisional, Kalender-Sync, Reschedule, Storno, Fotograf-Wechsel, Bestätigung, Payrexx-Webhook, Exxas-Order-Sync, Rate-Limiting & Security-Header, API-Key-Verwaltung (CRUD), **§19 Backend-CI** (`booking-ci.yml`, `npm test`, `lint:html`) |
| [docs/OPS_M365_GRAPH_BKBN.md](docs/OPS_M365_GRAPH_BKBN.md) | Betrieb M365 Graph + BKBN: `Calendars.Read`, Admin-Consent, VPS `BKBN_CALENDAR_MAILBOXES`, Verifikation (`npm run verify:graph-bkbn` in `booking/`) |
...
| [docs/FLOWS_UPLOAD.md](docs/FLOWS_UPLOAD.md) | Upload-System: Endpunkte, upload_batches/files-Tabellen, NAS-Pfad-Logik, produktiver NAS-Pull/rsync-Worker, Raw→Kundenordner-Background-Job, Chunked-Upload, Zeitstempel-Erhalt, Konflikt-Modi, Upload-Gruppen |
...
|---|---|
| `core` | `customers`, `customer_contacts`, `companies`, `company_members`, `admin_users`, `sessions`, `customer_sessions`, `customer_login_tokens` (Magic-Link, PR #443), `api_keys` |
...
| 6 | Exxas-Order-Sync (`exxas_status`, `exxas_order_id`) ohne dedizierte Dokumentation der Sync-Häufigkeit | ✅ **Behoben** April 2026 in `docs/FLOWS_EXXAS.md` §3 + §7: Rechnungs-Sync ist On-demand (kein Cron); Order-Sync-Felder vorhanden aber Runtime-Trigger noch nicht implementiert — explizit dokumentiert. |
