...
import {
  ...
  publicGalleryUrl,
  submitOfficeFollowupComment,
  type EmailTemplateVars,
} from "../galleryApi.ts";
import type { ClientGalleryRow, EmailTemplateRow, GalleryFeedbackRow } from "../types.ts";
...
export function GalleryRueckfrageModal({ gallery, customerComment, onClose, onSaved }: Props) {
  ...
  async function onSubmit() {
    ...
    setBusy(true);
    try {
      await submitOfficeFollowupComment({
      ...
      if (mail) {
        ...
        setOkNotice(
          "Rückfrage wurde in der Auswahl gespeichert. Es ist keine Kunden-E-Mail hinterlegt – bitte die Adresse bei dieser Auswahl nachtragen oder den Kunden manuell informieren. Sie können dieses Fenster schließen.",
        );
      }
    } catch (e) {
    ...
    <div className="gal-admin-modal-backdrop" role="presentation" onClick={onClose}>
      ...
      >
        ...
        <p className="admin-muted gal-admin-spacer">
          Der Text erscheint im <strong>Kommentarbereich</strong> in der Kundenansicht derselben Auswahl zum gleichen Bild.
        </p>
        <p className="gal-admin-rueckfrage-mail-hint">
          Der Kunde erhält <strong>zusätzlich eine E-Mail</strong>. Nach «Absenden» öffnet sich Ihr E-Mail-Programm mit dem Entwurf, sofern eine Kunden-E-Mail bei dieser Auswahl gespeichert ist.
        </p>
        <p className="admin-muted gal-admin-spacer">
