...
async function generateInvoicePdfBuffer(invoice, tour) {
  ...
  const invoiceDate = invoice.sent_at
    ? new Date(invoice.sent_at).toLocaleDateString('de-CH', { day: '2-digit', month: 'long', year: 'numeric' })
    : new Date().toLocaleDateString('de-CH', { day: '2-digit', month: 'long', year: 'numeric' });
  const dueDate = invoice.due_at ? new Date(invoice.due_at).toLocaleDateString('de-CH', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '-';
  ...
  return new Promise((resolve, reject) => {
    ...
    let y = 50;
    doc.fontSize(18).fillColor('#111').text(creditor.name, 50, y);
    y += 24;
    doc.fontSize(10).fillColor('#666').text(`${creditor.email} · ${creditor.website}`, 50, y);
    y += 30;
    doc.fontSize(14).fillColor('#111').text('Rechnung', 50, y);
    y += 22;
    doc.fontSize(10).fillColor('#666').text(`${invLabel} · ${invoiceDate}`, 50, y);
    y += 25;
    doc.fontSize(10).fillColor('#111').text('Rechnungsempfänger:', 50, y);
    y += 16;
    doc.text(tour.customer_name || tour.customer_contact || tour.kunde_kontakt || '-', 50, y);
    y += 16;
    if (tour.customer_email) { doc.text(tour.customer_email, 50, y); y += 20; } else { y += 10; }
    doc.fontSize(10).fillColor('#111').text('Tour / Objekt:', 50, y);
    y += 14;
    doc.fontSize(9).fillColor('#333').text(tourLabel, 50, y);
    y += 14;
    if (tourAddress) { doc.text(`Adresse: ${tourAddress}`, 50, y); y += 14; }
    if (tourLink) {
      doc.fillColor('#0b6aa2').text(`Link: ${tourLink}`, 50, y, { link: tourLink, underline: true });
      y += 16;
    }
    const tableTop = y + 5;
    ...
    try {
      ...
      console.warn('QR bill render failed in PDF generation:', err.message);
      y += 24;
      doc.fontSize(9).fillColor('#111').text(`Zahlbar an: ${creditor.name}`, 50, y);
      y += 14;
      doc.text(`IBAN: ${qrBill.formatIban(creditor.account)}`, 50, y);
      y += 14;
