...
**Manueller Export:** Admins können in der Auftragsliste (`app/src/components/orders/OrderTable.tsx`, `OrdersPage`) pro Zeile einen Exxas-Dienstleistungsauftrag anlegen. API: `POST /api/admin/orders/:orderNo/exxas-create-service-order` in `booking/server.js` (`requireAdmin`) → `POST {apiBase}/documents` mit `bezeichnung` = Adresse + `#Auftragsnr.`, `typ: "s"` und zwingendem `ref_kunde`. `ref_kunde` kommt ausschliesslich aus `core.customers.exxas_customer_id` (direkt am verknüpften Kunden oder Fallback per Billing-/Kontakt-E-Mail über `getCustomerByEmail()` inkl. Aliase); `exxas_contact_id` darf nur als optionales `ref_kontakt` gesendet werden. Fehlt die Exxas-Kunden-ID, bricht der Export lokal mit klarer Fehlermeldung ab, statt einen Exxas-HTTP-406 auszulösen. Erfolg → `setExxasOrderId`, Fehler → `setExxasError`. Kein vollautomatischer Trigger bei jeder Buchung, kein Cron-Job.
