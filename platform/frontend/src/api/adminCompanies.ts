...
export async function deleteAdminCompanyInvitation(token: string, companyId: number, invitationId: number) {
  return apiRequest<{ ok: true }>(
    `/api/admin/companies/${companyId}/invitations/${invitationId}`,
    "DELETE",
    token,
  );
}

export async function resendAdminCompanyInvitation(token: string, invitationId: number) {
  return apiRequest<{ ok: true; invitation: AdminInvitationRow }>(
    `/api/admin/users/invitations/${invitationId}/resend`,
    "POST",
    token,
  );
}
