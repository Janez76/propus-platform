...
function resolveCustomerPortalUrl() {
  const configured = String(import.meta.env.VITE_CUSTOMER_PORTAL_URL || "").trim();
...
export function Topbar({ onMenuToggle }: TopbarProps) {
  ...
  useEffect(() => {
    const configuredRaw = String(import.meta.env.VITE_CUSTOMER_PORTAL_URL || "").trim();
  ...
  const handleLogout = () => {
    clearAuth();
    const redirect = encodeURIComponent(new URL(import.meta.env.BASE_URL || "/", window.location.origin).toString());
