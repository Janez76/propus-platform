...
export default async function AssistantLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-classic)] [--assistant-sidebar-offset:272px]">
      <AppSidebar initialRole={session.role as Role} />
      <main className="min-h-screen min-w-0 p-3 lg:py-6 lg:pr-6 lg:pl-[calc(var(--assistant-sidebar-offset)+1.5rem)]">{children}</main>
    </div>
  );
}
