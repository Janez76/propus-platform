            ...
            {editingName && canManage ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="cust-form-input rounded-lg px-2 py-1 text-lg font-semibold"
                  autoFocus
                ...
                <button onClick={() => { setEditingName(false); setCompanyName(company?.name || ""); }} className="cust-action-icon p-1.5"><X className="h-4 w-4" /></button>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-semibold text-[var(--text-main)]">{company?.name || "Firmenportal"}</h1>
                {canManage && (
                  <button onClick={() => setEditingName(true)} className="cust-action-icon"><Pencil className="h-4 w-4" /></button>
                )}
              </div>
            )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-2 rounded-xl border border-slate-200 bg-white p-4 md:grid-cols-4 border-[var(--border-soft)] bg-[var(--surface)]">
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="cust-filter-select h-10">
          {statusOptions.map((s) => <option key={s} value={s}>{s === "alle" ? "Alle Status" : s}</option>)}
        </select>
        <select value={memberFilter} onChange={(e) => setMemberFilter(e.target.value)} className="cust-filter-select h-10">
          <option value="alle">Alle Mitarbeiter</option>
          {activeEmployees.map((m) => <option key={m.id} value={String(m.id)}>{m.email}</option>)}
        </select>
        <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="cust-filter-select h-10" />
        <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="cust-filter-select h-10" />
      </div>
                  ...
                  <div>
                    ...
                    <div className="mt-0.5 flex flex-wrap gap-1">
                      {ROLE_PERMISSIONS[m.role as CompanyMemberRole]?.map((p) => (
                        <span key={p} className="cust-badge cust-badge--neutral text-[10px]">{p}</span>
                      ))}
                    </div>
                  </div>
                </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 border-[var(--border-soft)] bg-[var(--surface)]">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <input
            ...
            disabled={inviteBusy}
            className="flex-1 cust-form-input rounded-lg text-sm placeholder:text-slate-400 focus:border-[var(--accent)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)] disabled:opacity-50 "
                ...
                <div key={inv.id} className="flex flex-col gap-2 rounded-lg border border-slate-100 bg-slate-50 px-4 py-3 border-[var(--border-soft)] bg-[var(--surface)]/60 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="btn-secondary min-h-0 flex items-center gap-1 px-2.5 py-1.5 text-xs"
                    >
