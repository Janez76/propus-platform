...
export function BildauswahlSendMailModal({ gallery, onClose, onRecordedSent }: Props) {
  ...
  const selectedTpl = templates.find((t) => t.id === BILDAUSWAHL_INVITE_TEMPLATE_ID) ?? templates[0];
            ...
            {showResendWarning ? (
              <p className="gal-admin-mail-send-modal__resend-warn" role="status">
                {g.client_delivery_sent_at ? (
                  <>
                    Der Kunde hat die Galerie bereits am <strong>{fmtDeliveryDate(g.client_delivery_sent_at)}</strong>{" "}
                    erhalten. Ein erneuter Versand könnte zu Verwirrung führen.
                  </>
                ) : (
                  <>
                    Für dieses Listing ist der Versand bereits als «versendet» markiert. Ein erneuter Versand könnte zu
                    Verwirrung führen.
                  </>
                )}
              </p>
