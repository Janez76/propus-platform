...
import {
  applyTemplateVars,
  BILDAUSWAHL_FOLLOWUP_TEMPLATE_ID,
  BILDAUSWAHL_REVISION_DONE_TEMPLATE_ID,
  listEmailTemplates,
  BILDAUSWAHL_INVITE_TEMPLATE_ID,
  saveEmailTemplate,
} from "../../../api/bildauswahlAdmin";
import type { EmailTemplateRow } from "../../../components/listing/types";
import { buildEmailPreviewSrcDoc } from "../listing/emailPreviewShell";
...
const TEMPLATE_OPTIONS: { id: string; label: string; placeholders: string[] }[] = [
  {
    id: BILDAUSWAHL_INVITE_TEMPLATE_ID,
    label: "Bestellung fertig",
    placeholders: [
      "{{gallery_link}}",
      "{{title}}",
      "{{customer_name}}",
  ...
  {
    id: BILDAUSWAHL_FOLLOWUP_TEMPLATE_ID,
  ...
  {
    id: BILDAUSWAHL_REVISION_DONE_TEMPLATE_ID,
    label: "Revision behoben",
    placeholders: [
      "{{customer_name_line}}",
      "{{asset_label}}",
      "{{revision}}",
...
export function BildauswahlEmailTemplatesPage() {
  const [templates, setTemplates] = useState<EmailTemplateRow[]>([]);
  const [selectedId, setSelectedId] = useState(BILDAUSWAHL_INVITE_TEMPLATE_ID);
  ...
  async function onSave() {
    ...
    setSaving(true);
    try {
      await saveEmailTemplate(selectedId, subject.trim(), body);
      setSaveOkFlash(true);
      await refresh();
    } catch (e) {
    ...
    <div className="admin-content gal-admin-listings-page gal-et-page">
      <div className="gal-admin-listings-shell__head">
        <div className="gal-admin-listings-shell__titles">
          ...
          <h1 className="gal-admin-listings-shell__h1">E-Mail an Kunden</h1>
          <p className="admin-lead">Vorlagen für den automatischen E-Mail-Versand.</p>
        </div>
        <button
          type="button"
          className={
