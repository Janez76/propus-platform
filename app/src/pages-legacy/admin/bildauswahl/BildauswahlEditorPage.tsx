...
import {
  ...
  BILDAUSWAHL_REVISION_DONE_TEMPLATE_ID,
  getGallery,
...
import { BildauswahlFeedbackMailModal } from "./BildauswahlFeedbackMailModal";
import { BildauswahlRueckfrageModal } from "./BildauswahlRueckfrageModal";
import { BildauswahlSendMailModal } from "./BildauswahlSendMailModal";
...
export function BildauswahlEditorPage() {
  ...
  const loadNasBrowser = useCallback(
    async (rootKind: "customer" | "raw" | "selection" | "selection", relativePath: string) => {
      if (!id || id === "new") return;
      setNasLoading(true);
      setNasError(null);
      try {
  ...
  const loadNasContext = useCallback(async () => {
    ...
    try {
      ...
      const nextRootKind = (context.currentSource.storage_root_kind ?? context.suggestions[0]?.rootKind ?? "customer") as "customer" | "raw" | "selection" | "selection";
      const nextRelativePath = context.currentSource.storage_relative_path ?? context.suggestions[0]?.relativePath ?? "";
      const healthKey =
        nextRootKind === "raw" ? "rawRoot"
        : nextRootKind === "selection" ? "selectionRoot"
        : "customerRoot";
  ...
  /** `/admin/bildauswahl/new` → Galerie anlegen und zur echten ID weiterleiten.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const targetPath = pathBildauswahlAdmin(gallery.id);
  ...
  /** Wenn die Galerie via `/admin/bildauswahl/new?orderNo=…` aus einer Bestellung
    ...
    return (
      <div className="admin-content gbe-editor-outer">
        <div className="gbe-page">
          ...
          <Link to={pathBildauswahlAdmin()} className="admin-link">
            Zur Übersicht
          </Link>
        </div>
      </div>
  ...
  const crumbCurrent = isNewListing ? "Neue Bildauswahl" : g.title;

  return (
    <div className="admin-content gbe-editor-outer">
      <div className="gbe-page">
        <p className="gbe-breadcrumb">
          <Link to={pathBildauswahlAdmin()}>Bildauswahl</Link>
        ...
        {/** Freigabe-Link + Matterport gehoeren zum Listing-Modul — Bildauswahl nutzt nur NAS-Import. */}
        ...
        {mailOpen && g ? (
          <BildauswahlSendMailModal gallery={g} onClose={() => setMailOpen(false)} onRecordedSent={() => void load()} />
        ) : null}
        {revisionMailFeedback && g ? (
          <BildauswahlFeedbackMailModal
            gallery={g}
            feedback={revisionMailFeedback}
            templateId={BILDAUSWAHL_REVISION_DONE_TEMPLATE_ID}
            title="Revision behoben – E-Mail"
            onClose={() => setRevisionMailFeedback(null)}
          />
        ) : null}
        {rueckfrageFor && g ? (
          <BildauswahlRueckfrageModal
            gallery={g}
