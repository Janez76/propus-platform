...
const WRITE_TOOL_LABELS: Record<string, string> = {
  ...
  create_customer_contact: "Kontaktperson erstellen",
  update_customer_note: "Kundennotiz aktualisieren",
  send_email: "E-Mail senden",
  draft_email_reply: "E-Mail-Antwort senden",
  create_listing_gallery: "Listing-Galerie erstellen",
  prepare_customer_delivery: "Kundenzustellung vorbereiten",
};
...
export async function runAssistantTurn(input: AssistantTurnInput): Promise<AssistantTurnResult> {
  ...
  if (input.forceModel) {
    ...
    currentTier = selectInitialModel(input.userMessage, maxTier);
  } else {
    currentTier = maxTier;
  }
  ...
  const modelId = input.forceModel || MODEL_IDS[currentTier];
  const firstResult = await executeWithModel(modelId);

  if (!autoEscalation || firstResult.pendingConfirmation) {
    const { _tier, ...result } = firstResult;
    return result;
  }

  const nextTier = shouldEscalate(
    { finalText: firstResult.finalText, toolCallsCount: firstResult.toolCallsExecuted.length },
    currentTier,
    maxTier,
  );
  ...
  const escalatedResult = await executeWithModel(MODEL_IDS[nextTier]);
  return {
    finalText: escalatedResult.finalText,
    history: escalatedResult.history,
    toolCallsExecuted: escalatedResult.toolCallsExecuted,
