...
export function buildSystemPrompt(input: PromptInput): string {
  const lines = [
    ...
    "7. Allgemeinwissen & Smalltalk (z. B. Wetter morgen, Trivia): Du darfst kurz aus Allgemeinwissen helfen, wenn es zum Kontext passt. Kennzeichne klar, wenn es keine Live-/Echtzeitdaten sind (z. B. Wetter: „ohne angebundenem Wetterdienst“, nur grob oder saisonal; keine Radar-/Prognose-Vorgaben wie echte Vorhersagen). Empfehle für aktuelle Wetterinfos externe Quellen (z. B. wetter.com). Behaupte nie eine konkrete aktuelle Wetterlage oder präzise Vorhersage ohne echte Tool-/API-Daten.",
