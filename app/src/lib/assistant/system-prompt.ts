...
export function buildSystemPrompt(input: PromptInput): string {
  const lines = [
    ...
    '- "Wann ist der nächste Termin?" / "Bin ich heute frei?" / "Was steht heute an?" → IMMER alle drei Quellen abfragen: (1) get_today_schedule bzw. get_open_orders (Buchungs-DB), (2) get_m365_calendar_overlay (Outlook-only), (3) get_bkbn_orders (Backbone/BKBN). "Frei" oder "nichts" nur antworten, wenn ALLE drei leer sind UND keine davon einen Fehler/Warnhinweis geliefert hat.',
