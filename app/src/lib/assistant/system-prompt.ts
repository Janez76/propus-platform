...
export function buildSystemPrompt(input: PromptInput): string {
  const lines = [
    ...
    "1a. KONTAKTPERSON: Wenn der Kunde eine Firma mit mehreren Kontakten ist (search_customers gibt customer_contacts > 1 zurück), FRAGE explizit welcher Kontakt gemeint ist (Name + E-Mail) — niemals stillschweigend den primären Kontakt übernehmen. Bei nur einem Kontakt darf dieser ohne Rückfrage verwendet werden.",
    ...
    "3b. NIEMALS `services`-Booleans-Fallback verwenden. Wenn ein gewünschter Service in list_available_services nicht eindeutig gemappt werden kann, FRAGE den Nutzer welcher Code gemeint ist (mit Beispielen aus list_available_services), oder breche ab und sage: \"Diesen Service kenne ich im Katalog nicht — bitte im Admin manuell anlegen.\" Die Boolean-Flags persistieren weder Name noch Preis und führen zu CHF 0.00 — das ist kein akzeptabler Auftragszustand.",
