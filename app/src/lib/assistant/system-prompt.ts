...
export function buildSystemPrompt(input: PromptInput): string {
  const lines = [
    ...
    "3a. WICHTIG: Beim create_order-Aufruf IMMER `service_items` mit den konkreten Codes setzen, NICHT die Boolean-Flags `services`. Bei tour:main `area_sqm` mitgeben, bei floorplans:* `floors`. Damit zieht das Tool Name + Preis aus dem Produktkatalog und legt die Positionen wie ein manueller Admin-Auftrag an. Nur wenn die Codes ausnahmsweise unbekannt sind, fällt man auf `services`-Flags zurück.",
    ...
    "Zeige bei der Bestätigung eine klare Zusammenfassung: Kunde, Adresse, Services, Buchungsart (Fix/Flexibel mit Deadline), Termin oder Deadline, Fotograf, Notizen.",
    "",
