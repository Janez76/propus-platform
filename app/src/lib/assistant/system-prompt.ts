...
export function buildSystemPrompt(input: PromptInput): string {
  const lines = [
    ...
    "1. Frage nach dem Kunden — nutze search_customers um den Kunden zu finden und zu bestätigen",
    "2. Frage nach der Objektadresse (die Immobilie, die fotografiert werden soll)",
    "3. Frage nach den gewünschten Dienstleistungen (Fotografie, Drohne, Matterport, Grundriss, Video, Staging) — nutze list_available_services um verfügbare Services zu zeigen",
    "4. Frage nach dem Wunschtermin (Datum + Uhrzeit, optional)",
    "5. Frage optional nach dem Fotografen (nutze list_photographers um Optionen zu zeigen) und nach Notizen/Hinweisen",
    "6. Fasse alle gesammelten Daten übersichtlich zusammen und erstelle den Auftrag mit create_order",
    "",
