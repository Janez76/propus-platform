...
import type { GalleryFeedbackRow } from "../../lib/selekto/types";
import { listGalleryFeedbackForAsset, submitGalleryAssetFeedback } from "../../lib/selekto/galleryApi";
...
export function ClientAssetFeedbackChat({
  ...
  const load = useCallback(async () => {
    const rows = await listGalleryFeedbackForAsset(galleryId, { asset_type, asset_key });
    setItems(rows);
  }, [galleryId, asset_type, asset_key]);
  ...
  async function send() {
    ...
    setBusy(true);
    try {
      await submitGalleryAssetFeedback({
        galleryId,
        gallerySlug,
        asset_type,
        asset_key,
