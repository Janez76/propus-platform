...
/** Basis-URL für WebDAV-Requests (Same-Origin-Proxy in Dev, optional gleicher Ursprung in Prod). */
function requestOriginForWebdav(): string {
  if (process.env.NODE_ENV === "development") {
    return "/__propus-nextcloud";
  }
  const proxy = (process.env.NEXT_PUBLIC_NEXTCLOUD_PROXY as string | undefined)?.trim();
  if (proxy) {
...
export function devPublicDavProxyUrl(originalRemote: string): string | null {
  ...
  try {
    ...
    const raw = process.env.NEXT_PUBLIC_NEXTCLOUD_PROXY_ALLOW_HOSTS as string | undefined;
    const allowed = raw
      ...
      return {
        ...
        message:
          process.env.NODE_ENV === "development"
            ? "WebDAV konnte nicht erreicht werden (Netzwerk). Prüfen Sie den Proxy."
            : "Der Browser blockiert WebDAV (CORS). Nutzen Sie lokal «npm run dev», setzen Sie NEXT_PUBLIC_NEXTCLOUD_PROXY auf einen gleichlautenden Reverse-Proxy, oder legen Sie eine gallery.json auf die Freigabe.",
      };
    }
    return {
      ok: false,
